...
function projectCard(p) {
  ...
  return [
    ...
    `<img src="${p.image || "assets/img/projects/.gitkeep"}" alt="" style="width:100%;height:140px;object-fit:cover;border-radius:8px;">`,
    `<h3>${p.title || "Untitled"}</h3>`,
    `<p>${p.description || ""}</p>`,
    `<div class="tags">${tags}</div>`,
    p.url ? `<p><a class="btn" href="${p.url}" target="_blank" rel="noopener">Open</a></p>` : "",
    `</article>`
  ].join("");
}
